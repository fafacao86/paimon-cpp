...
Result<std::function<Status(const Literal&)>> LiteralSerializationUtils::CreateValueWriter(
    ...
    switch (field_type) {
        ...
        default:
            return Status::Invalid(fmt::format("Unsupported field type for literal serialization: {}",
...
Result<std::function<Result<Literal>()>> LiteralSerializationUtils::CreateValueReader(
    ...
    switch (field_type) {
        ...
        default:
            return Status::Invalid(fmt::format("Unsupported field type for literal deserialization: {}",
                                               FieldTypeUtils::FieldTypeToString(field_type)));
    }
}
