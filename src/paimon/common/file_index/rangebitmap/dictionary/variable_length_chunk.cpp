...
VariableLengthChunk::VariableLengthChunk(const Literal& key, int32_t code, int32_t offset,
...
VariableLengthChunk::VariableLengthChunk(const std::shared_ptr<InputStream>& input_stream,
...
Result<PAIMON_UNIQUE_PTR<Bytes>> VariableLengthChunk::Serialize(MemoryPool* pool) const {
  // Calculate sizes
  int32_t header_size = sizeof(int8_t) + serializer_->SerializedSizeInBytes(key_) +
                        4 * sizeof(int32_t);
