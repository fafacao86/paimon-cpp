...
Result<std::unique_ptr<KeyFactory>> KeyFactory::Create(MemoryPool* pool, const FieldType field_type) {
    switch (field_type) {
...
Result<std::unique_ptr<Chunk>> FixedLengthKeyFactory::CreateChunk(const Literal& key, const int32_t code,
...
Result<std::unique_ptr<Chunk>> FixedLengthKeyFactory::MmapChunk(
    ...
    const int32_t chunk_offest,
    const int32_t keys_base_offset) {
    ...
    PAIMON_ASSIGN_OR_RAISE(const auto key_literal, deserializer(data_in, nullptr));
    ...
    return std::make_unique<FixedLengthChunk>(key_literal, code, offset, size,
        this->shared_from_this(), input_stream, keys_base_offset, keys_length, fixed_length);
}
    ...
    return KeyDeserializer(
        [this](const std::shared_ptr<DataInputStream>& in, MemoryPool* pool) -> Result<Literal> {
            PAIMON_ASSIGN_OR_RAISE(auto reader,
                                  LiteralSerializationUtils::CreateValueReader(GetFieldType(), in, pool));
            return reader();
        });
}

Result<std::unique_ptr<Chunk>> VariableLengthKeyFactory::CreateChunk(const Literal& key, int32_t code,
