...
Result<std::shared_ptr<FileIndexWriter>> RangeBitmapFileIndex::CreateWriter(
  ...
  // Create writer - this would need the field name and schema
  // For now, return an error as we need more context
  return Status::NotImplemented("RangeBitmapFileIndex::CreateWriter needs field information");
}
...
Result<std::shared_ptr<RangeBitmapFileIndexReader>> RangeBitmapFileIndexReader::Create(
  ...
  PAIMON_ASSIGN_OR_RAISE(auto range_bitmap,
                         RangeBitmap::Deserialize(input_stream, start,
