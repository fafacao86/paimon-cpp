...
class IndexFileMetaV1Deserializer : public ObjectSerializer<std::shared_ptr<IndexFileMeta>> {
 public:
            ...
            {arrow::field("_INDEX_TYPE", arrow::utf8(), false),
             ...
             arrow::field("_DELETIONS_VECTORS_RANGES",
                          arrow::list(arrow::field(
                              "item",
                              arrow::struct_({arrow::field("f0", arrow::utf8(), false),
                                              arrow::field("f1", arrow::int32(), false),
    ...
    explicit IndexFileMetaV1Deserializer(const std::shared_ptr<MemoryPool>& pool)
    ...
    Result<BinaryRow> ToRow(const std::shared_ptr<IndexFileMeta>& meta) const override {
        assert(false);
        return Status::Invalid("IndexFileMetaV1Deserializer to row is not valid");
    }
