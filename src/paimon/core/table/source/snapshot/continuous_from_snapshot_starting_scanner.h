...
namespace paimon {
/// `StartingScanner` for the `StartupMode::FromSnapshot()` startup mode of a
/// streaming read.
class ContinuousFromSnapshotStartingScanner : public StartingScanner {
 public:
    ContinuousFromSnapshotStartingScanner(const std::shared_ptr<SnapshotManager>& snapshot_manager,
    ...
    Result<std::shared_ptr<ScanResult>> Scan(
        ...
        return std::make_shared<StartingScanner::NextSnapshot>(
            std::max(starting_snapshot_id_.value(), earliest_id.value()));
    }
};
}  // namespace paimon
